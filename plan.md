...
- `backend/app/config/` 作为后端配置包，集中管理 env 加载与结构化 settings；embedding 配置已独立到子模块
...
- Env:
  - `.env` 存储服务端密钥与连接配置
  - `backend/app/config/` 负责集中加载 env；数据库当前主契约为 `POSTGRES_*`
...
- `article_id` 作为业务唯一键，`canonical_url` 作为数据库幂等键
