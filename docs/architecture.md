...
- `app/`：放 API、domain service、任务入口与配置
- `app/config/`：集中维护模型与服务配置，例如 embedding 与 Milvus
- `app/core/`：集中维护数据库与 Redis 等稳定基础设施；`database.py` 维护 engine/session/Base
- `app/models/`：集中放 SQLAlchemy ORM models，按领域拆分
...
- `app/service/document_ingestion_service.py`：负责把采集结果去重后写入 PostgreSQL `document`，并直接操作 ORM / Session
- `test/`：统一存放 API、数据模型、脚本与聚合回归测试
...
9. 基于本轮新增文档生成新的不可变 `story` 与 `story_article`
...
- `document_asset`、Milvus、image enrichment、story 聚合、feed JSON 导出仍未接入这条新 service 路径

### Story 聚合链路

1. 读取上次成功运行之后新增的文档
2. 基于新增文档生成 cluster
3. 为本轮产出生成新的 `story_key`
4. 写入不可变 `story` 与 `story_article`
5. 生成首页读模型输出
...
- 用户可能收藏或继续讨论某个 story
- story 范围内聊天需要跨天连续
- citation 与 session 恢复必须稳定

模型定义：

- `story_key`：稳定 story 标识
- `story`：不可变 story 聚合主记录
- `story_article`：不可变成员关系
...
- Feishu 是最终唯一登录入口
- 当前阶段认证链路 deferred，先打通内容与检索主链路
- 接入后仅允许 allowlist 组织访问
- 登录结果必须写审计记录
