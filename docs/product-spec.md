...
- Feishu 正式登录链路
- tenant allowlist 与登录审计
- 以 PostgreSQL 为真相源的文档、story、session、memory 持久化
- 以 Milvus 为检索层的 `content_unit` 与 `user_memory`
- story 内 AI 问答与全局 AI sidebar
- 历史 session 恢复
- 通过 `published_run` 发布和回滚
- 前端从静态 JSON 切换到 API
...
- 连续性优先：用户讨论的是稳定的 story，不是某次 run 的临时聚类结果
...
- 不让前端 URL 或会话恢复依赖 per-run snapshot ID
- 不把产品改造成聊天优先的交互形态
...
用户通过 Feishu 登录进入产品。服务端验证 tenant 是否在 allowlist 中，并记录本次登录尝试。
...
- 对外稳定标识是 `story_key`
- 默认读取当前 `published_run`
- 需要支持按需回放某个 `run_id` 下的视图
...
- 必须携带 `story_key` 与 `scope_snapshot_run_id`
- 回答必须带 citation
...
- story 范围内会话要能绑定具体 snapshot 做回放
...
- `story_key` 是稳定身份
- `run_id` 是发布版本，不是长期 story 身份
- story 成员关系以 SQL 快照为准，不以向量库冗余字段为准
...
- story 范围消息必须记录 `scope_snapshot_run_id`
...
- 让用户看到的是经过校验、可回滚、连续可追踪的 story 版本

需求约束：

- `sources.yaml` 继续作为采集配置真相源
- 发布不能覆盖历史结果，必须通过切换 `published_run`
- 重聚类时尽量复用既有 `story_key`

## 10. 核心对象与术语

- `story_key`：稳定 story 身份，对外 URL、session、引用都依赖它
- `run_id`：某次聚合与发布的快照版本标识
- `published_run`：当前线上默认读取的发布版本
- `document`：原始文档主记录
- `document_asset`：文档相关图片、视频等资产引用
- `content_unit`：检索单元，粒度是 chunk 或资产，不是 article 粒度
...
- 用户讨论的对象是 story，不是 cluster snapshot
- 用户回放的是某次 run 下的 story 视图，不是临时前端状态
...
- PostgreSQL：用户、登录事件、文档、story、session、citation、memory、run 元数据真相源
- Milvus：`content_unit`、`user_memory`、`user_profile_memory` 的检索副本
- `feed-data.json`：迁移期前端兜底产物，不是长期真相源

## 11.3 Story 边界

- `story_key` 是稳定标识
- `(run_id, story_key)` 才表示某次发布视图中的 story
- `story_cluster_snapshot` 不是长期主表
...
- `content_unit` 是内容检索核心 collection
- 检索按 chunk 或资产粒度建模
- story 成员关系以 SQL 快照为准，Milvus 中的 story 字段只做冗余过滤
...
5. 生成 retrieval units 并写入 Milvus。
6. 生成新的 story 快照。
7. 执行校验后切换 `published_run`。
8. 在迁移期继续产出静态 feed 兜底结果。

## 12.2 首页读取链路

1. 前端请求 `GET /api/v1/feed/home`。
2. 后端读取当前 `published_run`。
...
1. 用户打开某个 story。
2. 用户发起 story 范围提问。
3. 后端读取 story 上下文和对应 snapshot。
4. 后端补充检索相关 `content_unit`。
...
- 用户可以通过 Feishu 正常登录，且访问控制可审计
- 首页仍然是稳定、清晰、快速的信息流主入口
- story 详情足以支撑快速阅读与继续分析
- AI 回答具备上下文、citation 和回溯能力
- 用户能够恢复历史 session
- 长期记忆可以提升后续问答相关性
- story 在跨 run 更新时仍保持连续身份
- 发布失败时可通过切换 `published_run` 回滚
...
- 文档进入 PostgreSQL
- 生成 retrieval units
- 建立 `content_unit` 检索能力

## 15.4 Phase 3：Story 发布层
...
- 发布链路在 v1 是否允许人工审批，还是完全自动切换 `published_run`
...
- 先确认产品问题、目标用户和 v1 范围
- 再确认 story、memory、retrieval 和发布模型等关键架构边界
